<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot').'module/inventarisasi_bhp/business/PopupKodeBarang.class.php';
...
class PopupKodeBarang extends HtmlResponse
{
   ...
   function TemplateModule()
   {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
         'module/inventarisasi_bhp/template');
   ...
   function ProcessRequest()
   {
      ...
      Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'cari_golongan', array('CARI_GOLONGAN', $GolList, $CARI_GOLONGAN, '', 'id="POP_UP_GOLONGAN"'), Messenger::CurrentRequest);

      $JnsBarangList = $Obj->GetComboJnsBarang();
		Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'cari_jenis_barang', array('CARI_JENIS_BARANG', $JnsBarangList, $CARI_JENIS_BARANG, '', ''), Messenger::CurrentRequest);
      // ---------

      // Inisialisasi link URL
      $return['url']['search'] = Dispatcher::Instance()->GetUrl('inventarisasi_bhp','kodeBarang','popup','html');
      ...
      $totalData = $Obj->GetDataBarangCount($return['cari']);
      $itemViewed = 20;
      $currPage = 1;
	...
	function ParseTemplate($data = NULL)
   {
      ...
      {
			// Kelompok
         if (!isset($data['barang'][$idKelompok]) OR count($data['barang'][$idKelompok]) === 0)
            continue;
         ...
         foreach ($data['subKelompok'][$idKelompok] as $idSubKelompok=>$value)
			{
				// Sub Kelompok
            if (!isset($data['barang'][$idKelompok][$idSubKelompok]) OR count($data['barang'][$idKelompok][$idSubKelompok]) === 0)
               continue;
            ...
            $this->mrTemplate->parseTemplate('data_item', 'a');
   			...
   			{
   				// Barang
					//if (!isset($data['barang'][$idKelompok][$idSubKelompok][$idBarang]) OR count($data['barang'][$idKelompok][$idSubKelompok][$idBarang]) === 0)
               //continue;
