...
$sql['get_count_data_unitkerja'] = "
	...
	FROM unit_kerja_ref
		...
		*/
	WHERE 
		(unitkerjaKode/*LPAD(if(tempUnitKode IS NULL,unitkerjaKode*100,tempUnitKode*100+unitkerjaKode),4,'0')*/ LIKE '%s' OR LPAD((if(tempUnitKode IS NULL,unitkerjaKode,tempUnitKode))*100,4,'0') LIKE '%s' )
		AND (unitkerjaNama LIKE '%s' OR tempUnitNama LIKE '%s') AND (unitkerjaId = %s OR unitkerjaParentId = %s OR '1' = %s) 
 ";
