...
require_once GTFWConfiguration::GetValue( 'application', 'docroot').
   'module/' . Dispatcher::Instance()->mModule . '/business/PopupKodeBarang.class.php';

//set Label for module
require_once GTFWConfiguration::GetValue( 'application', 'docroot') .
'module/label/response/Label.proc.class.php';

class ViewPopUpKodefikasi extends HtmlResponse
{
	...
	function TemplateModule()
	{
		...
		$this->SetTemplateFile('pop_up_kodefikasi.html');
	}
         ...
         foreach ($data['subKelompok'][$idKelompok] as $idSubKelompok=>$value)
            ...
            $this->mrTemplate->parseTemplate('data_item', 'a');
               ...
               $send = $value;
   				$send['number'] = '';
   				$send['class_name'] = '';
					$data['nama'] = $this->filter($value['nama']);
					$this->mrTemplate->AddVars('data_item', $send, 'DATA_');
   				$this->mrTemplate->parseTemplate('data_item', 'a');
