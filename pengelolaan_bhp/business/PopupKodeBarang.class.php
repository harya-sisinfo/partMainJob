...
{
   protected $mSqlFile= 'module/pengelolaan_bhp/business/popup_kode_barang.sql.php';
   ...
   function GetUserKodeSistem(){
      $userId = Security::Instance()->mAuthentication->GetCurrentUser()->GetUserId();
      $result = $this->Open($this->mSqlQueries['get_user_kode_sistem'], array($userId));
   ...
   {
      $kdSistem = $this->GetUserKodeSistem();
      extract ($data);
   ...
   function GetDataJenis($data, $tglRth, $unitId, $start = 0, $limit = 1000)
   {
      $kdSistem = $this->GetUserKodeSistem();
      extract ($data);
      if (is_numeric($CARI_GOLONGAN))
      ...
      (
         ...
         "%$CARI_NAMA_BARANG%",
         $CARI_JENIS_BARANG,
         $kdSistem,$kdSistem.'.%', // FIXME: Belum menghandle child akses BHP parent, parent bisa ambil semua dari child
         $tglRth,
         $unitId,
         $start,
         $limit
