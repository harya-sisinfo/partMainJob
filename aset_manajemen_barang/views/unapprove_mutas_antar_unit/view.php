					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<div class="input-daterange input-group" id="periode-range">
								<input type="text" class="form-control" name="tglAwal" placeholder="Tanggal Awal" value="<?php echo (!empty($tglAwal_))?$tglAwal_:date('Y-').'01-01' ?>" autocomplete="off">
								<span class="input-group-addon">s.d</span>
								<input type="text" class="form-control" name="tglAkhir" placeholder="Tanggal Akhir" value="<?php echo (!empty($tglAkhir_))?$tglAkhir_:date('Y-m-d') ?>" autocomplete="off">
							</div>
						</div>
					</div>
					<div class="form-group">
...
<div class="panel">
	...
	<div class="panel-body">
		<div class="table-light table-primary" id="table-container">
			<table cellpadding="0" cellspacing="0" border="0" width="100%" class="table table-striped table-bordered" id="jq-datatables-aset-unapprove-mutasi-antar-unit">
				<thead>
					<tr>
...
<script>
	$(document).ready(function(){
		...
		$("#jq-datatables-aset-unapprove-mutasi-antar-unit").tooltip({
			selector: '[data-toggle="tooltip"]'
		});

		dtTable = $('#jq-datatables-aset-unapprove-mutasi-antar-unit').DataTable({
			"processing": false,
			"serverSide": true,
			"scrollCollapse": true,
			"order": [[ 1, "DESC" ]],
			"ajax": {
				...
				error: function () {
					$(".jq-datatables-aset-unapprove-mutasi-antar-unit-error").html("");
					$("#jq-datatables-aset-unapprove-mutasi-antar-unit").append('<tbody class="jq-datatables-aset-unapprove-mutasi-antar-unit-error"><tr><th colspan="9">No data found in the server</th></tr></tbody>');
		...
		$('#jq-datatables-aset-mutasi_wrapper-unapprove-antar-unit .table-caption').text('List Data');
		$('#jq-datatables-aset-mutasi_wrapper-unapprove-antar-unit .dataTables_filter input').attr('placeholder', 'Cari...');
		$("#jq-datatables-aset-unapprove-mutasi-antar-unit").on("click", ".btn-unapprove", function (e) {
            e.preventDefault();
            var self = $(this);
            bootbox.confirm({
                title: 'Konfirmasi',
                message: "Yakin data akan diunapprove ?",
