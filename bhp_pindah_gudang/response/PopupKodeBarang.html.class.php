...
class PopupKodeBarang extends HtmlResponse
{
   ...
   function ProcessRequest()
   {
	  ...
	  # init filter by unit, gedung, and usulan asal
	  $unitId = $_GET['unitid'];
     ...
     $tgl = $_GET['tgl'];
	  ...
	  if($unitId == '' || $gudangId == ''){
	  	echo "Pilih Unit dan Gudang Asal terlebih dahulu";
	  	exit;
	  }
      ...
      $return['url']['search'] = Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule,'KodeBarang','popup','html').'&unitid='.$unitId.'&gudangid='.$gudangId.'&usulid='.$usulId.'&tgl='.$tgl;
      ...
      $url = Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule, Dispatcher::Instance()->mSubModule, Dispatcher::Instance()->mAction, Dispatcher::Instance()->mType) . '&cari=' . Dispatcher::Instance()->Encrypt($cari).'&unitid='.$unitId.'&gudangid='.$gudangId.'&usulid='.$usulId.'&tgl='.$tgl;
	  $dest = "popup-subcontent";
	  $jenis = $Obj->GetDataJenis($return['cari'], $unitId, $gudangId, $tgl, $usulId, $startRec, $itemViewed);
	  $return['jenis'] = $jenis;
